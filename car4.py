...
def check_all_mess(mes):
    ...
    response('The available colours are : \n1.White\n2.Black\n3.Silver\n4.Blue\n5.Brown', ['colour' 'colours'])
    response('The available colours are : \n1.White\n2.Black\n3.Silver\n4.Blue\n5.Brown ', [ 'color', 'colors'])
    ...
    response('It has a mileage of 17 to 23 kmpl', ['how', 'much', 'is', 'the','mileage'], required_words=['mileage'])
    response('It has a fuel tank capacity of 45 litres.', ['what', 'is', 'the', 'fuel' , 'tank','capacity'], required_words=['fuel'])
    ...
    response('It is equipped with voice control.', ['does', 'it', 'have', 'voice','control'], required_words=['voice'])
    ...
    response('Models available are:\n\t1.Venue E\n\t2.Venue S\n\t3.Venue SX ', ['what', 'models', 'are', 'available'], required_words=['models'])
    response('Models available are \n\t1.Venue E\n\t2.Venue S\n\t3.Venue SX', ['what', 'model', 'is', 'available'], required_words=['model'])
    response('Price range for this car is 7 lacs to 12 lacs', ['what', 'is', 'price', 'range'], required_words=['price'])
    ...
    response('It Has Maximum speed of 140kmph', ['speed'], required_words=['speed'])
    response('It Has 525 Horsepower(HP) petrol and diesel engine', ['engine'], required_words=['engine'])
