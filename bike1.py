...
def check_all_mess(mes):
    ...
    response('Rs.1.20,000 - Rs.1,90,000', ['what','price','range'], required_words=['price'])
    response('30 - 40km/l on an average of all the models.', ['average','mileage'], required_words=['mileage'])
    response('The available colours are : \n\t1.White\n\t2.Black\n\t3.Silver\n\t4.Blue\n\t5.Red\n\t6.Golden\n', ['colors', 'color', 'colours', 'colour'])
    response('It has leather seats.', ['does', 'it', 'have', 'leather','seats'], required_words=['seats'])
    response('It has a fuel tank capacity of 13 to 13.5 litres.', ['what', 'is', 'the', 'fuel' , 'tank','capacity'], required_words=['fuel','capacity'])
    ...
    response('The average weight is 186kg to 191kg.', ['what', 'is', 'the', 'weight'], required_words=['weight'])
    ...
    response('It Has Cubic Capacity of 350', ['engine'], required_words=['engine'])
